...
{
    spi_init(STM_SPI, 1 * 1000 * 1000);
    spi_set_format(STM_SPI, 8, SPI_CPOL_0, SPI_CPHA_1, SPI_MSB_FIRST);
...
{
    uart_init(STM_UART, 115200);
    gpio_set_function(STM_PIN_UART_TX, GPIO_FUNC_UART);
    gpio_set_function(STM_PIN_UART_RX, GPIO_FUNC_UART);
    uart_set_format(STM_UART,8,1,UART_PARITY_EVEN);
}

uint32_t stm_spi_sync()
{
    spi_write_read_blocking(STM_SPI, cmd_blhello, tmp_in_buff, 4);
...
uint32_t stm_spi_is_unlock()
{
    ...
    sleep_us(1000);
    spi_write_read_blocking(STM_SPI, cmd_rmem, tmp_in_buff, 3);
    spi_write_read_blocking(STM_SPI, cmd_rmem1, tmp_in_buff, 3);
    if (tmp_in_buff[1] == 0x79)
    {
        log_info_str("ACK_CMD", 7);
        printf("resp cmd = %x\r\n", tmp_in_buff[1]);
        sleep_us(1000);
        spi_write_read_blocking(STM_SPI, cmd_rmem_addr_pkt, tmp_in_buff, 5);
        spi_write_read_blocking(STM_SPI, cmd_rmem1, tmp_in_buff, 3);
        if (tmp_in_buff[1] == 0x79)
        {
            printf("resp addr = %x\r\n", tmp_in_buff[1]);
            sleep_us(1000);
            spi_write_read_blocking(STM_SPI, cmd_rmem_nbyte_pkt, tmp_in_buff, 2);
            spi_write_read_blocking(STM_SPI, cmd_rmem1, tmp_in_buff, 3);
            if (tmp_in_buff[1] == 0x79)
            {
                printf("resp size = %x\r\n", tmp_in_buff[1]);
                sleep_us(1000);
                spi_write_read_blocking(STM_SPI, mbuff, mbuff, 257);
                printf("DUMP DATA\r\n");
...
{
    clr_uart_fifo(STM_UART);
    uart_putc(STM_UART, 0x7F);
    uint8_t c = getc_uart(STM_UART, 1000);
    clr_uart_fifo(STM_UART);
    if (c == 0x79)
        return 1;

    uart_putc(STM_UART, 0x7F);
    c = getc_uart(STM_UART, 1000);
    clr_uart_fifo(STM_UART);
    if (c == 0x79)
        return 1;
...
uint32_t stm_uart_is_unlock()
{
    ...
    uint8_t xor = (addr_1 ^ addr_2 ^ addr_3 ^ addr_4);
    clr_uart_fifo(STM_UART);
    uart_putc(STM_UART, 0x11);
    uart_putc(STM_UART, 0xEE);
    c = getc_uart(STM_UART, 500);
    d = getc_uart(STM_UART, 500);
    e = getc_uart(STM_UART, 500);
    if (c == 0x79 && d == 0x00)
    {
        log_info_str("ACK_CMD", 7);
        printf("resp cmd = %x\r\n", c);
        uart_putc(STM_UART, addr_1);
        uart_putc(STM_UART, addr_2);
        uart_putc(STM_UART, addr_3);
        uart_putc(STM_UART, addr_4);
        uart_putc(STM_UART, xor);
        c = getc_uart(STM_UART, 1000);
        d = getc_uart(STM_UART, 1000);
        if (c == 0x79 && d == 0x00)
        {
            printf("resp addr = %x\r\n", c);
            uart_putc(STM_UART, 0xFF);
            uart_putc(STM_UART, 0x00);
            c = getc_uart(STM_UART, 400);
            if (c == 0x79)
            {
                printf("resp size = %x\r\n", c);
                printf("DUMP DATA\r\n");
                ...
                for (int i = 0; i < 0x100; i++)
                {               
                    app.memory[i] = uart_getc(STM_UART);
                    printf("IND = 0x%02x , val = %02x\r\n", i, app.memory[i]);                    
                }
                log_info_str("DUMP", 4);
                printf("\r\nDUMP DONE\r\n");
